...
- **Aplikasi web terpasang (PWA, baru)** — bisa **di-install** ke layar utama HP/desktop (Chrome/Edge/Safari: *Add to Home Screen / Install*) dan **dibuka offline** (app-shell di-cache service worker). Kartu **Target Belajar Harian** punya tombol **🔔 Aktifkan pengingat** untuk notifikasi saat ada soal review jatuh tempo.
...
- **Tanda "⚠️ Yakin tapi salah" (baru, metakognisi)** — saat kamu menjawab dengan keyakinan **💪 Yakin** tapi ternyata **keliru**, soal itu ditandai khusus: ini miskonsepsi paling berbahaya (kamu kira sudah paham). Aplikasi **menaikkan prioritas review**-nya, menampilkan badge di Jurnal Kesalahan, dan menyediakan drill **"⚠️ Yakin tapi salah"** di Latihan. Tanda otomatis **lepas begitu kamu menjawabnya benar**. Dasar: *hypercorrection effect* — kesalahan berkeyakinan tinggi justru paling cepat diperbaiki bila langsung ditarget.
...
- **Evaluasi per sub-topik (baru)** — di bawah tiap mata uji di Statistik muncul **heatmap sub-topik** (mis. *Deret 83% · Peluang 63%*, terlemah di depan, warna hijau/kuning/merah). Klik chip untuk **latih topik itu** langsung. Bila soal punya **tingkat kesulitan**, aplikasi membedakan celah **fondasi** (gagal di soal mudah → tombol *Buka materi* yang langsung menyorot topik terkait) dari **siap naik level** (dasar oke, jatuh di soal sulit → tombol *Latih sulit*). Sub-topik & kesulitan ditandai admin di Input Soal (atau via field `subtopic`/`difficulty` saat import; atau lewat **Input Soal → 🏷️ Tandai massal** yang menandai banyak soal sekaligus dengan saran sub-topik otomatis). Saat **jawab salah** di Latihan/Hasil, muncul tautan **📘 Perkuat topik ini** langsung ke materi sub-topik terkait. Heatmap baru muncul setelah ada cukup data per topik.
